...
- db-migration的migrate子命令帮助信息

```bash
./db-migration migrate -h
database migrations tool

Usage:
  db-migration migrate [flags]
  db-migration migrate [command]
...
Use "db-migration migrate [command] --help" for more information about a command.
```
...
# 只需对生成的该go文件做修改
# 对于新加的migration，需要重新编译  服务，才能包含并可执行新的migration操作
# 通过命令行参数指定gorm模式：-m gorm 或 --mode gorm (也可不指定，默认为该模式)
$ ./db-migration migrate create -n init_schema
Generated new migration files:
./cmd/  /db-migration/migrations/20230520120159_init_schema.go

# 为了便于演示，已经用上面同样的方式另外创建了两个测试用的migration，name参数分别为test_mig001和test_mig002
# 查看当前db的迁移状态，3个pending代表有三个migration待做迁移
$ ./db-migration migrate status -c /tmp/data_service.yaml
...
$ ./db-migration migrate create -n init_schema -m sql
Generated new migration files:
./cmd/  /db-migration/migrations/20230207215606_init_schema.go
./cmd/  /db-migration/migrations/sql/20230207215606_init_schema_up.sql
./cmd/  /db-migration/migrations/sql/20230207215606_init_schema_down.sql
...
```bash
# 分步迁移，如向前迁移1个版本
$ ./db-migration migrate up -s 1 -c /tmp/data_service.yaml
Connecting to MySQL database...
Database connected!
...
# 直接迁移到最新版本
./db-migration migrate up -c /tmp/data_service.yaml
Connecting to MySQL database...
Database connected!
...
```bash
# 分步回滚，如向后回滚1个版本
$ ./db-migration migrate down -s 1 -c /tmp/data_service.yaml
Connecting to MySQL database...
Database connected!
...
$ ./db-migration migrate up -s 1 -d -c /tmp/data_service.yaml
Connecting to MySQL database...
Database connected!
Running migration 20230511114513

2023/05/20 12:09:50  .io/cmd/  /db-migration/migrations/20230511114513_add_template.go:121
[1.358ms] [rows:-] SELECT DATABASE()

2023/05/20 12:09:50  .io/cmd/  /db-migration/migrations/20230511114513_add_template.go:121
[4.871ms] [rows:1] SELECT SCHEMA_NAME from Information_schema.SCHEMATA where SCHEMA_NAME LIKE 'bk_ _admin%' ORDER BY SCHEMA_NAME='bk_ _admin' DESC,SCHEMA_NAME limit 1

2023/05/20 12:09:50  .io/cmd/  /db-migration/migrations/20230511114513_add_template.go:121
[4.914ms] [rows:-] SELECT count(*) FROM information_schema.tables WHERE table_schema = 'bk_ _admin' AND table_name = 'template_spaces' AND table_type = 'BASE TABLE'
...
- migrate的up、down、status子命令都需要连接mysql，所以需要用-c参数指定  的配置文件，用于获取mysql配置
- 对于新加的migration，需要重新编译  服务，才能包含并可执行新的migration操作
-   的migrate create命令需要在 源码根目录下运行，才能正常运行且保证生成的migration相关文件在正确位置
- migrate create指定的migration名称，中划线'-'会被转化成下划线'_'，以保持migration相关文件名称格式统一# db-migration
