...
func genGormMigrationFile(version, fileName string, in interface{}) error {
	...
	t := template.Must(template.ParseFiles("./cmd/  /db-migration/migrator/gorm_schema.tmpl"))
	err := t.Execute(&out, in)
	if err != nil {
		return errors.New("Unable to execute template:" + err.Error())
	}

	migrationFile, err := os.Create(fmt.Sprintf("./cmd/  /db-migration/migrations/%s_%s.go",
		version, fileName))
	if err != nil {
...
func genSqlMigrationFile(version, fileName string, in interface{}) error {
	...
	t := template.Must(template.ParseFiles("./cmd/  /db-migration/migrator/sql_schema.tmpl"))
	err := t.Execute(&out, in)
	if err != nil {
		return errors.New("Unable to execute template:" + err.Error())
	}
	...
	upSQLFile, err := os.Create(fmt.Sprintf("./cmd/  /db-migration/migrations/sql/%s_%s_up.sql",
		version, fileName))
	if err != nil {
	...
	downSQLFile, err = os.Create(fmt.Sprintf("./cmd/  /db-migration/migrations/sql/%s_%s_down.sql",
		version, fileName))
	if err != nil {
